...
- Windows: `laping-windows-x86_64.zip`（`laping.exe` と `libcurl-x64.dll` が同梱されています）
...
Windowsの場合は `laping-windows-x86_64.zip` を展開し、中の `laping.exe` と `libcurl-x64.dll` を**同じフォルダ**に置いたまま使います（DLLが無いと起動できません）。
